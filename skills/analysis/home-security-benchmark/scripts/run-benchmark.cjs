...
const AEGIS_TOOLS = [
    { type: 'function', function: { name: 'video_search', description: 'Search security camera clips by date, camera, or description', parameters: { type: 'object', properties: { query: { type: 'string' }, date: { type: 'string' }, camera: { type: 'string' } } } } },
    { type: 'function', function: { name: 'video_analyze', description: 'Analyze a specific video clip using VLM', parameters: { type: 'object', properties: { clip_id: { type: 'string' }, question: { type: 'string' } } } } },
